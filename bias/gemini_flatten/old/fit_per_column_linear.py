...
if __name__ == "__main__":
    ...
    for side in ['top', 'bot']:
        for c in empty_cols:
            ...
            A = np.vstack([np.ones(len(S_arr)), S_arr]).T
    ...
    # Save
    np.savez('gemini_flatten/params_per_col_linear.npz', 
             betas_top=results['top']['betas'], sigmas_top=results['top']['sigmas'],
             betas_bot=results['bot']['betas'], sigmas_bot=results['bot']['sigmas'])
    print("Per-column linear parameters saved.")
