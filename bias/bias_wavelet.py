...
def wavelet_destripe(image, wavelet='haar', levels=7, tuning_factor=1.5):
    """
    ...
    This algorithm works well in images where the columns are only slightly perturbed from the usual offset (all CaF2/SrF2, off-nadir LyaN/LyaX)
    """
...
def wavelet_destripe_log(image, wavelet='haar', levels=7, tuning_factor=1.5):
    """
    Removes vertical striping using Wavelet Domain Median Subtraction in log space (works better since gain sag is also stripey).
    Preserves global DC offset.
    Uses Haar wavelet since the target is vertical stripes.
    Levels indicate how deep to go. Will crash if 2^levels < npix.
    Tuning factor controls the amount of thresholding - the higher the threshold, the more coefficients are removed.

    This algorithm works well in images where the column are heavily perturbed (on-nadir LyaN/LyaX)
    """
    # Add offset so it doesn't crash
    offset = np.abs(np.nanmin(image)) + 1.0
    log_image = np.log(image + offset)
    # Decompose the image into Approximation (LL) and Details (LH, HL, HH)
    coeffs = pywt.swt2(log_image, wavelet=wavelet, level=levels)
    N = log_image.size
    ...
    log_final = pywt.iswt2(modified_coeffs, wavelet=wavelet)
    final_image = np.exp(log_final) - offset
    recovered_stripes = final_image - image
    return final_image, recovered_stripes
...
def display_transforms(channel, im_type):
    ...
    if im_type == 'dark' or im_type == 'oob_off_nadir':
        ...
        if channel == 'NFI':
            im_1, stripes_1 = remove_dark_stripes(final_image, channel)
            clean_image, stripes_2 = wavelet_destripe_log(im_1, wavelet='haar', levels=5, tuning_factor=1)
            recovered_stripes = stripes_1 + stripes_2
        else:
            clean_image, recovered_stripes = wavelet_destripe_log(final_image, wavelet='haar', levels=4, tuning_factor=1.7)
