...
def generate_datafile(dates_chunk):
    ...
    if store.getsize() > 0:
        # if dataset exists already, append to it
        ds.to_zarr(store=store, append_dim='date')
    else:
        # otherwise, create it
...
for dates_chunk in tqdm(dates_chunks):
    generate_datafile(dates_chunk)
