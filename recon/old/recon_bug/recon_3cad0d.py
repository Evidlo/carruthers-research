...
for num_obs, win, season, cpoints, t_op, L in items:
    ...
    grid = DefaultGrid((500, 45, 60), size_r=(3, 25), spacing='log')
    m = Zoennchen24Model(grid=grid, device=device); mstr = 'zoennchen'
    from pratik_mod import Pratik25Model
    m = Pratik25Model(grid=grid, num_times=1, device=device); mstr = 'pratik'
    ...
    loss_fns = [
        # 1 * SquareLoss(projection_mask=fr.proj_maskb),
        1 * AbsLoss(projection_mask=fr.proj_maskb),
        # 1e2 * SquareRelLoss(projection_mask=fr.proj_maskb),
        1e4 * NegRegularizer(),
    ...
    meas = meas * fr.proj_maskb
    retrieved_meas = retrieved_meas * fr.proj_maskb
    nonzero = meas != 0
